...
const results=[];
function record(name,passed,details=''){results.push({name,passed:Boolean(passed),details});if(!passed)console.error(`FAIL: ${name}${details?` — ${details}`:''}`)}
...
const click=(selector,index=0)=>evaluate(`(()=>{const el=document.querySelectorAll(${js(selector)})[${index}];if(!el)return false;el.click();return true})()`);
const text=(selector,index=0)=>evaluate(`document.querySelectorAll(${js(selector)})[${index}]?.textContent?.trim()||''`);

// Desktop functional suite.
await viewport(1440,1000,false);await navigate();
...
record('No horizontal page overflow on desktop',await evaluate('document.documentElement.scrollWidth<=document.documentElement.clientWidth+1'));
const desktopSticky=await evaluate("(async()=>{const editor=document.querySelector('.editor-card'),card=document.querySelector('#previewCard'),minimum=card.style.minHeight,smooth=document.documentElement.style.scrollBehavior;card.style.minHeight='1800px';document.documentElement.style.scrollBehavior='auto';window.scrollTo({top:editor.offsetTop+160,behavior:'instant'});await new Promise(resolve=>setTimeout(resolve,80));const header=document.querySelector('.topbar').getBoundingClientRect(),ribbon=editor.getBoundingClientRect();const result={headerBottom:header.bottom,ribbonTop:ribbon.top,scrollY,scrollHeight:document.documentElement.scrollHeight};window.scrollTo(0,0);card.style.minHeight=minimum;document.documentElement.style.scrollBehavior=smooth;return result})()");
record('Desktop header scrolls away',desktopSticky.headerBottom<=0,JSON.stringify(desktopSticky));
record('Desktop Arrangement Tools remains pinned to the top',Math.abs(desktopSticky.ribbonTop)<=1,JSON.stringify(desktopSticky));

await click('.chord-family .chord',3);await click('.drop-target',0);
record('Place chord through palette click',await text('.placed-chord',0)==='Cmaj7');
await click('#tabSlash');
await evaluate("(()=>{for(const [id,value] of [['slashRoot','D'],['slashQuality','m'],['slashBass','F#']]){const el=document.getElementById(id);el.value=value;el.dispatchEvent(new Event('change',{bubbles:true}))}return true})()");
await click('#addSlashBtn');await click('#slashChordBank .chord',0);await click('.drop-target',1);
record('Create and place slash chord',await text('.placed-chord',1)==='Dm/F#');

await click('#tabNashville');
const numberCounts=await evaluate("[...document.querySelectorAll('.nashville-number-row')].map(row=>row.querySelectorAll('.nashville-key').length)");
record('Nashville rows include normal/lower/upper/zero',JSON.stringify(numberCounts)===JSON.stringify([7,7,7,1]),JSON.stringify(numberCounts));
await click('.nashville-row-block:nth-child(3) .nashville-key',3);await click('#nashvilleChordBank .chord',3);await click('.drop-target',2);
record('Place upper-octave Nashville chord',(await text('.placed-chord',2)).replaceAll('•','').includes('4maj7'));
record('Upper-octave dot remains rendered',await evaluate("!!document.querySelectorAll('.placed-chord')[2]?.querySelector('.nashville-octave-high .nashville-octave-dot')"));

await click('#tabRhythm');await click('.duration-option',0);await click('.drop-target',4);
record('Half-beat creates two subdivisions',(await evaluate("document.querySelectorAll('.duration-half .sub-beats')[0]?.children.length"))===2);
await click('.sub-beat',1);
record('Nested half-beat creates a two-child branch',(await evaluate("document.querySelectorAll('.nested-sub-beat').length"))===2);
...
record('Lyrics mode creates fields for every base/sub beat',(await evaluate("document.querySelectorAll('.lyric-input').length"))>=16);
record('Lyrics fields use an empty placeholder',await evaluate("[...document.querySelectorAll('.lyric-input')].every(input=>input.placeholder==='')"));
await evaluate("(()=>{const input=document.querySelector('.lyric-input');input.value='Grace';input.dispatchEvent(new Event('input',{bubbles:true}));window.print=()=>{};document.querySelector('#exportBtn').click();return true})()");
...
await navigate();await click('.chord-family .chord',3);await click('.drop-target',0);await click('#tabNashville');await click('#nashvilleChordBank .chord',0);await click('.drop-target',1);await click('#transposeUp');
...
await evaluate("(()=>{const meter=document.querySelector('#timeSignature');meter.value='3/4';meter.dispatchEvent(new Event('change',{bubbles:true}));return true})()");
record('Time signature updates beat count',(await evaluate("document.querySelectorAll('.drop-target').length"))===12);
await click('#previewTitle');await evaluate("(async()=>{const input=document.querySelector('#previewTitle input');input.value='Mobile Grace';input.dispatchEvent(new FocusEvent('blur'));await new Promise(resolve=>setTimeout(resolve,20));return true})()");
const editedTitle=await evaluate("({preview:document.querySelector('#previewTitle').textContent.trim(),source:document.querySelector('#songTitle').value})");
record('Inline title editing works',editedTitle.preview==='Mobile Grace'&&editedTitle.source==='Mobile Grace',JSON.stringify(editedTitle));

// Export and import project file without triggering a real download.
await evaluate("(()=>{window.prompt=()=> 'regression-score';window.__downloadName='';window.__exportBlob=null;URL.createObjectURL=blob=>(window.__exportBlob=blob,'blob:regression');URL.revokeObjectURL=()=>{};HTMLAnchorElement.prototype.click=function(){window.__downloadName=this.download};document.querySelector('#saveBtn').click();return true})()");
const exported=await evaluate("(async()=>({name:window.__downloadName,data:JSON.parse(await window.__exportBlob.text())}))()");
record('Export .file uses the expected extension',exported.name==='regression-score.chordsheet.json',exported.name);
record('Export .file contains metadata, meter, sections and beats',exported.data.title==='Mobile Grace'&&exported.data.meter==='3/4'&&Array.isArray(exported.data.sections)&&!!exported.data.sections[0].beats,JSON.stringify({title:exported.data.title,meter:exported.data.meter,sections:exported.data.sections?.length,hasBeats:!!exported.data.sections?.[0]?.beats}));
await evaluate(`(async()=>{const project=${JSON.stringify({format:'chord-sheet',version:2,title:'Imported Song',artist:'Regression Artist',key:'F',chordRoot:'F',customChord:'F6/9',meter:'4/4',lyricsEnabled:true,sections:[{id:'imported',name:'Verse',lyricsEnabled:true,lyricBeats:{'0-0':'Sing','0-1:1':'grace','0-2:0':'through','0-2:3':'night','0-3:0':'for','0-3:1':'me','1-0:0.0':'nested','1-0:0.1':'half','1-1':'slash'},bars:4,beats:{'0-0':{chord:'F',duration:null},'0-1':{chord:null,duration:'half'},'0-1:1':{chord:'2̇m',duration:null},'0-2':{chord:null,duration:'quarter'},'0-2:0':{chord:'1̇',duration:null},'0-2:3':{chord:'3̣',duration:null},'0-3':{chord:null,duration:'half'},'0-3:0':{chord:'4̇',duration:null},'0-3:1':{chord:'1̇',duration:null},'1-0':{chord:null,duration:'half'},'1-0:0':{chord:null,duration:'half'},'1-0:0.0':{chord:'4',duration:null},'1-0:0.1':{chord:'Gmaj7',duration:null},'1-0:1':{chord:'7',duration:null},'1-1':{chord:'C/F',duration:null}}}],slashChords:['F/A'],nashvilleNumber:'2̇',nashvilleAccidental:''})};const input=document.querySelector('#projectFileInput'),file=new File([JSON.stringify(project)],'import.chordsheet.json',{type:'application/json'}),transfer=new DataTransfer();transfer.items.add(file);input.files=transfer.files;input.dispatchEvent(new Event('change',{bubbles:true}));await new Promise(resolve=>setTimeout(resolve,50));return true})()`);
...
record('PDF nested half-beat markers follow parent and child content widths',[...nestedLineSizing.parents,...nestedLineSizing.children].every(row=>Math.abs(row.line-row.notes)<.6),JSON.stringify(nestedLineSizing));
await printPdf('mixed-chord-nashville-lyrics');await cdp.send('Emulation.setEmulatedMedia',{media:'screen'});await screenshot('desktop-1440');
...
await fs.writeFile(`${outputDir}/results.json`,JSON.stringify({appUrl,total:results.length,passed:results.length-failed.length,failed:failed.length,results},null,2));
