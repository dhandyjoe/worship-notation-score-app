...
function chordName(quality) {
   return `${state.chordRoot}${quality}`;
}
...
function isNashvilleChord(value) {
   return /^[♭#]?[0-7][̣̇]?/u.test(String(value));
}
function validChordSuffix(suffix) {
...
function bindPaletteItem(element, item) {
   ...
   if (selectedPaletteItem?.type === item.type && selectedPaletteItem?.value === item.value)
      element.classList.add("palette-selected");
...
function renderCustomChord() {
   const value = state.customChord.trim();
   $("#customChordPreview").innerHTML = value
      ? `<span class="chord custom-chord" draggable="true" data-chord="${value}">${value}</span>`
      : '<span class="custom-chord-hint">Custom chord preview</span>';
   bindDraggableChords();
}
function meterInfo() {
   const [top] = state.meter.split("/").map(Number);
   return { beats: top };
}
function normalizeSection(section, meter = "4/4") {
   const bars = Math.max(1, Number(section.bars) || 4),
      beats = section.beats && typeof section.beats === "object" ? section.beats : {},
      lyricBeats = {};
...
function renderControls() {
   ...
   $("#slashChordBank").innerHTML = state.slashChords
      .map((chord) => `<span class="chord slash-chord" draggable="true" data-chord="${chord}">${chord}</span>`)
      .join("");
   ...
   document
      ...
      .forEach((option) => bindPaletteItem(option, { type: "duration", value: option.dataset.duration }));
}
function syncEditor() {}
function beatValue(section, slot) {
...
function chordOrDot(section, slot) {
   ...
   return value.chord
      ? `<span class="placed-chord" role="button" tabindex="0" title="Tap to remove" aria-label="Remove ${escapeHTML(value.chord)} chord">${chordLabel(value.chord)}</span>`
      : '<span class="beat-dot">·</span>';
}
function lyricValue(section, slot) {
...
function sectionHTML(section) {
   ...
   const bars = Array.from(
      { length: section.bars },
      (_, bar) =>
         `<div class="bar ${showLyrics ? "has-lyrics" : ""}" style="--beats:${beats}" data-bar="${bar}"><button class="delete-bar" type="button" data-section="${section.id}" data-bar="${bar}" title="Delete bar ${bar + 1}" aria-label="Delete bar ${bar + 1}">×</button>${Array.from({ length: beats }, (_, beat) => beatHTML(section, bar, beat)).join("")}</div>`,
   );
   ...
   const title =
      section.id === state.editingId
         ? `<input class="section-title-input" data-section="${section.id}" value="${section.name}" aria-label="Section name">`
         : `<button class="section-title" data-section="${section.id}" title="Click to edit section name">${section.name.toUpperCase()}</button>`;
   const lyricsToggle =
...
function bindPreview() {
   document.querySelectorAll(".drop-target").forEach((beat) => {
      ...
      beat.addEventListener("drop", (event) => {
         ...
         const section = state.sections.find((item) => item.id === beat.dataset.section),
      ...
      beat.addEventListener("click", (event) => {
         if (
            !selectedPaletteItem ||
            event.target.closest(".placed-chord,.duration-line,.nested-duration-line,.lyric-input")
         )
            return;
         event.stopPropagation();
         const section = state.sections.find((item) => item.id === beat.dataset.section),
   ...
   document.querySelectorAll(".placed-chord").forEach((chord) => {
      const removeOrReplace = (event) => {
         ...
         const beat = chord.closest(".drop-target"),
            section = state.sections.find((item) => item.id === beat.dataset.section);
   ...
   document.querySelectorAll(".nested-duration-line").forEach((line) =>
      line.addEventListener("click", (event) => {
         event.stopPropagation();
         const section = state.sections.find((item) => item.id === line.dataset.section),
   ...
   document.querySelectorAll(".duration-line").forEach((line) =>
      line.addEventListener("click", (event) => {
         ...
         const group = line.closest(".beat-group"),
            section = state.sections.find((item) => item.id === group.dataset.section),
   ...
   document.querySelectorAll(".lyric-input").forEach((input) => {
      ...
      input.addEventListener("input", () => {
         const section = state.sections.find((item) => item.id === input.dataset.section);
         if (!section) return;
      ...
      input.addEventListener("paste", (event) => {
         ...
         available.forEach((field, index) => {
            ...
            const section = state.sections.find((item) => item.id === field.dataset.section);
            const value =
   ...
   document.querySelectorAll(".section-lyrics-toggle").forEach((button) =>
      button.addEventListener("click", (event) => {
         event.stopPropagation();
         const section = state.sections.find((item) => item.id === button.dataset.section);
         if (!section) return;
...
function applyProject(project) {
   ...
   state = {
      ...
      nashvilleAccidental: ["", "♭", "#"].includes(project.nashvilleAccidental) ? project.nashvilleAccidental : "",
   ...
   $("#timeSignature").value = state.meter;
   syncEditor();
   renderControls();
   renderPreview();
   save();
...
$("#chordRootPicker").addEventListener("click", (event) => {
   ...
   clearPaletteSelection();
   state.chordRoot = event.target.dataset.root;
   renderControls();
   save();
});
$("#customChordInput").addEventListener("input", (event) => {
   clearPaletteSelection();
   state.customChord = event.target.value;
   renderCustomChord();
   save();
});
$("#addSlashBtn").addEventListener("click", () => {
   const chord = `${$("#slashRoot").value}${$("#slashQuality").value}/${$("#slashBass").value}`;
   if (!state.slashChords.includes(chord)) state.slashChords.push(chord);
   renderControls();
   save();
...
$("#nashvilleRootPicker").addEventListener("click", (event) => {
   ...
   if (!button) return;
   clearPaletteSelection();
   state.nashvilleNumber = button.dataset.number;
   renderControls();
   save();
});
$("#nashvilleAccidentalPicker").addEventListener("click", (event) => {
   ...
   clearPaletteSelection();
   state.nashvilleAccidental = event.target.dataset.accidental;
   renderControls();
   save();
});
...
$("#lyricsEnabled").addEventListener("change", (event) => {
   state.lyricsEnabled = event.target.checked;
   renderControls();
   renderPreview();
   save();
   toast(state.lyricsEnabled ? "Lyrics mode enabled" : "Lyrics hidden from score");
});
...
$("#timeSignature").addEventListener("change", (event) => {
   ...
   renderPreview();
   save();
   toast(`Preview updated to ${state.meter}`);
});
...
$("#resetSheetBtn").addEventListener("click", () => {
   ...
   const firstSection = newSection("Intro");
   state = {
      key: "C",
      chordRoot: "C",
      customChord: "",
